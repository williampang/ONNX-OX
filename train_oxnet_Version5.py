...
class PNGFolder(Dataset):
    def __init__(self, root, classes=None, target_size=None, transform=None):
        ...
        self.samples = []
        self.targets = [] # Keep track for weighting
...
def train_one_epoch(model, loader, opt, device, criterion, scheduler=None):
    model.train()
    running_acc = 0.0
    running_loss = 0.0
...
def eval_epoch(model, loader, device, criterion):
    ...
    class_correct = list(0. for i in range(len(loader.dataset.classes)))
    class_total = list(0. for i in range(len(loader.dataset.classes)))
    ...
    for i in range(len(loader.dataset.classes)):
        if class_total[i] > 0:
            print(f"  {loader.dataset.classes[i]}: {100 * class_correct[i] / class_total[i]:.2f}%")
...
def main():
    ...
    parser.add_argument("--onnx", type=str, default="model/model_v5.onnx")
    ...
    # Data Augmentation - slightly enhanced
    ...
    for i in range(len(train_ds.classes)):
        ...
        w = total_samples / (len(train_ds.classes) * count) if count > 0 else 0
        weights.append(w)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        tr_loss, tr_acc = train_one_epoch(model, train_loader, opt, device, criterion, scheduler)
